                    ...
                    @if(count($m->submenu) > 0)
                      <li class="dropdown">
                        ...
                        <ul class="dropdown-menu pull-right" role="menu">
                          ...
                          <li><a href="{{route('recruiting-article', ['slug' => $submenu->slug])}}">{{$submenu->name}}</a></li>
                          @endforeach
                          <li><a href="{{route('recruiting-article', ['slug' => $m->slug])}}">All {{$m->name}}</a></li>
                        </ul>
                      </li>
                    @else
                      <li class="{{ Request::url() === route('article', ['slug' => $m->slug]) ? '' : ''}}">
                        <a href="{{route('recruiting-article', ['slug' => $m->slug])}}">{{$m->name}}</a>
                      </li>
                    @endif
                  @endforeach
               </ul>
              ...
              <div class="post-body">
                <div class="entry-header">
                  <h2 class="entry-title">
                    <a href="{{route('recruiting-article-detail', ['slug' => $lead->slug])}}">
                      {{$lead->title}}
                    </a>
                  </h2>
                  <div class="post-meta">
                    <span class="post-author">
                      <i class="fa fa-bookmark"></i> {{$lead->category_name}}
                    </span>
                ...
                <div class="post-footer">
                  <a href="{{route('recruiting-article-detail', ['slug' => $lead->slug])}}" class="btn btn-primary">Read More</a>
                </div>
              </div><!-- post-body end -->
            </div><!-- 1st post end -->
            @endforeach
